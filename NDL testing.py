...
def basic_observations():
    ...
    table.state_name.value_counts().plot(kind = 'bar')
    plt.title('States and their City counts')
    ...
    table.groupby('state_name').dist_code.nunique().plot(kind = 'bar')
    plt.title('States and their District counts')
    plt.xlabel('Name of the states')
...
def district_code_operations():
    ...
    x = table.dist_code.index
    y = table.dist_code.values
    table.dist_code.plot(); plt.title('Cities vs District Frequency')
    plt.xlabel('Cities in alphabetical order')
    plt.ylabel('Frequency')
    plt.fill_between(x, y, 0, alpha = 0.11, facecolor = 'r')
    plt.show()
    
    #Plot that shows District numbers and their frequency
    table.dist_code.value_counts().sort_index().plot(kind='bar')
    plt.title('District Numbers vs Frequency')
    plt.xlabel('District Numbers')
    plt.ylabel('Frequency')
    plt.show()
